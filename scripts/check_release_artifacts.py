...
This script verifies that the repository contains the key files referenced by
README_REPRODUCE.md and ARTIFACT_MANIFEST.md. By default it checks released
code, configs, scripts, and frozen result summaries. Runtime processed JSONL
inputs under data/processed/ are checked only when --strict-data is supplied,
because they may be regenerated from the builder scripts.
"""
...
class Artifact:
    path: str
    kind: str = "file"  # "file", "dir", or "glob"
    required: bool = True
    note: str = ""
...
def _check_group(root: Path, title: str, artifacts: Iterable[Artifact]) -> tuple[int, int, list[str]]:
    missing_required = 0
    checked = 0
    lines: list[str] = [f"\n[{title}]"]
    for artifact in artifacts:
        ...
        lines.append(f"  {status:16s} {artifact.path}")
        if artifact.note:
            lines.append(f"    note: {artifact.note}")
        if artifact.required and not ok:
            missing_required += 1
    return checked, missing_required, lines
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict-data",
        action="store_true",
        help="Also require generated data/processed/*.jsonl runtime inputs to exist.",
    )
    ...
    for title, artifacts in groups.items():
        checked, missing, lines = _check_group(root, title, artifacts)
    ...
    if args.json:
        ...
        if not args.strict_data:
            print("\nNote: generated data/processed/*.jsonl inputs were not required. Use --strict-data after generating/restoring them.")
